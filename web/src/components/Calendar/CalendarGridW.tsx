      ...
      weekDays.forEach((day, dayIndex) => {
        ...
        if (eventStart < dayEnd && eventEnd > dayStart) {
          ...
          processed.push({
            event,
            dayIndex,
            top: (startMinutes / 60) * 44, // 44px per hour
            height: Math.max((durationMinutes / 60) * 44, 20) // 최소 20px
          });
        }
      });
    });
                  ...
                  return (
                    <div
                      ...
                      className={`
                        ...
                        ${bar.isFirst || shouldShowText ? styles.first : ''}
                        ${bar.isLast || dayIndex === bar.dayIndex + bar.width - 1 ? styles.last : ''}
                        ${bar.continueLeft && dayIndex === bar.dayIndex ? styles.continueLeft : ''}
                        ${bar.continueRight && dayIndex === bar.dayIndex + bar.width - 1 ? styles.continueRight : ''}
                      `}
                      style={{
            ...
            {weekDays.map((day, dayIndex) => (
              <div key={`time-${dayIndex}`} className={styles.timeDayColumn}>
                ...
                {processedTimedEvents
                  .filter(timedEvent => timedEvent.dayIndex === dayIndex)
